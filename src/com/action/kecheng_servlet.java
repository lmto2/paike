...
public class kecheng_servlet extends HttpServlet
{
	...
	{
        String actuibtype=req.getParameter("type");
		
		if(actuibtype.endsWith("kechengMana"))
		{
			kechengMana(req, res);
		}
		if(actuibtype.endsWith("kechengAdd"))
		{
			kechengAdd(req, res);
		}
		if(actuibtype.endsWith("kechengDel"))
		{
			kechengDel(req, res);
		}
		if(actuibtype.endsWith("kechengEdit"))
		{
			kechengEdit(req, res);
		}
		
		if(actuibtype.endsWith("kechengAll"))
		{
			kechengAll(req, res);
		}
		if(actuibtype.endsWith("kechengByStu"))
		{
	...
	public void kechengByStu(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				kecheng.setName(OrmService.getKechengName(rs.getInt("kecheng_id")));
				kechengList.add(kecheng);
		    }
			rs.close();
		}
